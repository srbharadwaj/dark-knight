...
public class chessPiece implements chessConstants {
    ...
    //public int enpassentPos = 0; //Applicable only to pawn 0-no enpassent else enpassent target square(FEN)
    public int b_currentPosition;
    public ArrayList b_movesPossible = null;

    public chessPiece(chessBoard o, String pName, String pColor, int iniPos, int curPos) {
        cbo = o;
        pieceName = pName;
        pieceColor = pColor;
    ...
    public void calculatePossibleMovesForPawn() {
        ...
        if (getPieceColor().equals(WHITE)) {
            ...
            if(cbo.getEnPassentPosition()==newpos){
                chkPositionValidityAndAddThePositionForPawn(newpos, false);
            }
    ...
    public boolean chkPositionValidityAndAddThePositionForPawn(int newpos, boolean capPos) {
        if (isValidPositionOnBoard(newpos)) {
            ...
            if (!capPos) {
                ...
                if (isOppositeColorPiecePresent(newpos, getPieceColor())) {
                    // add the position
                    // coz different color piece is present and can be captured
                    movesPossible.add(newpos);
                    return true;
                } else {
                    // dont add the position
            ...
            //Not a valid position on board so dont addd
            return false;
        }
    ...
    public boolean isOppKingPresentAtAdjacentBox(int newpos, String colour) {
        if (((isValidPositionOnBoard(newpos + 9))) && (isOppositeColorKingPresent(newpos + 9, colour))) {
            return true;
        }
        if (((isValidPositionOnBoard(newpos + 10))) && (isOppositeColorKingPresent(newpos + 10, colour))) {
            return true;
        }
        if (((isValidPositionOnBoard(newpos + 11))) && (isOppositeColorKingPresent(newpos + 11, colour))) {
            return true;
        }
        if (((isValidPositionOnBoard(newpos - 9))) && (isOppositeColorKingPresent(newpos - 9, colour))) {
            return true;
        ...
        if (((isValidPositionOnBoard(newpos + 1))) && (isOppositeColorKingPresent(newpos + 1, colour))) {
            return true;
        }
        if (((isValidPositionOnBoard(newpos - 1))) && (isOppositeColorKingPresent(newpos - 1, colour))) {
            return true;
    ...
    public boolean isValidPositionOnBoard(int pos) {
        /**
    ...
    public String getPossibleMovesWithPieceName() {
        String s = this.getPieceName() + " --> ";
        for (int i = 0; i < movesPossible.size(); i++) {
            s = s + movesPossible.get(i) + " ";
        }
        System.out.println(s);
        return s;
    }

    public String getCompletePieceInfo() {
        String s = getPieceColor() + " " + getPieceName() + " on " + this.getCurrentPosition() + " --> ";
        for (int i = 0; i < movesPossible.size(); i++) {
            s = s + movesPossible.get(i) + " ";
        }
        System.out.println(s);
    ...
    public int getEnPassentValue() {
        ...
        if ((pCol.equals(WHITE)) && (curPos >= 51) && (curPos <= 58)) {
            for (int j = 0; j < cbo.allPieces.size(); j++) {
                chessPiece p = (chessPiece) cbo.allPieces.get(j);
                if ((p.getPieceName().equals(PAWN))
                        && (p.getPieceColor().equals(BLACK))
                        && (p.movesDone.size() == 1)) {
                    if (p.getCurrentPosition() == (curPos + 1)) {
                        if (cbo.getLatestMove() == (curPos + 1)) {
                            return curPos + 11;
                        }
                    }
                    if (p.getCurrentPosition() == (curPos - 1)) {
